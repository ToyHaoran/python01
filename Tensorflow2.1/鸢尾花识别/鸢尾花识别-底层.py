...
for epoch in range(epoch):  # 数据集级别的循环，每个epoch循环一次数据集
    for step, (train_data, train_label) in enumerate(train_db):  # batch级别的循环 ，每个step循环一个batch
        with tf.GradientTape() as tape:  # with结构记录梯度信息
            # 前向传播过程 计算y
            y = tf.matmul(train_data, w1) + b1  # y为预测结果
            y = tf.nn.softmax(y)  # 使输出y符合概率分布(此操作后与独热码同量级，可相减求loss)
            y_ = tf.one_hot(train_label, depth=3)  # 将标签值转换为独热码格式，方便计算loss和accuracy
    ...
    for test_data, test_label in test_db:
        y = tf.matmul(test_data, w1) + b1  # 使用更新后的参数进行预测
        y = tf.nn.softmax(y)
        pred = tf.argmax(y, axis=1)  # 返回y中最大值的索引，即预测的分类
        pred = tf.cast(pred, dtype=test_label.dtype)  # 将pred转换为test_label的数据类型
        # 若分类正确，则correct=1，否则为0，将bool型的结果转换为int型
        correct = tf.cast(tf.equal(pred, test_label), dtype=tf.int32)
